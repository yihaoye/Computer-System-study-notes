...
根据这一点，可以把I/O设备当做文件来处理。因此，与磁盘上的普通文件进行交互所用的同一系统调用可以直接用于I/O设备。
...
int open(char *pathname, int flags, mode_t mode); //返回：若成功则为新文件描述符，若出错为－1
//open是UNIX系统（包括LINUX、Mac等）的系统调用函数，pathname参数是待打开/创建文件的POSIX路径名（如/home/user/a.cpp），
//flag参数指明如何访问该文件，mode参数指定新文件的访问权限仅在创建新文件时使用（即使用了O_CREAT时）
...
umask(DEF_UMASK); //每个进程都有个umask，用来设置访问权限
